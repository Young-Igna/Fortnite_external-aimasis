...
class Aimbot:
    ...
    def __init__(self, box_constant = 500, collect_data = False, mouse_delay = 0.0001, debug = False):
        #controls the initial centered box width and height of the "klaver Vision" window
        ...
        if torch.cuda.is_available():
            print(colored("CUDA ACCELERATION [ENABLED]", "magenta"))
        else:
    ...
    def update_status_aimbot():
        if Aimbot.aimbot_status == colored("ENABLED", 'magenta'):
            Aimbot.aimbot_status = colored("DISABLED", 'white')
        else:
            Aimbot.aimbot_status = colored("ENABLED", 'magenta')
    ...
    def is_aimbot_enabled():
        return True if Aimbot.aimbot_status == colored("ENABLED", 'magenta') else False
    ...
    def start(self):
        ...
        while True:
            ...
            if len(results.xyxy[0]) != 0: #player detected
                ...
                if closest_detection: #if valid detection exists
                    ...
                    if Aimbot.is_target_locked(absolute_head_X, absolute_head_Y):
                        cv2.putText(frame, "LOCKED", (x1 + 40, y1), cv2.FONT_HERSHEY_DUPLEX, 0.5, (115, 244, 113), 1) #draw the confidence labels on the bounding boxes
                    else:
                        cv2.putText(frame, "TARGETING", (x1 + 40, y1), cv2.FONT_HERSHEY_DUPLEX, 0.5, (115, 113, 244), 1) #draw the confidence labels on the bounding boxes
            ...
            cv2.putText(frame, f"FPS: {int(1/(time.perf_counter() - start_time))}", (5, 30), cv2.FONT_HERSHEY_DUPLEX, 1, (113, 116, 244), 1)
            cv2.imshow("klaver Vision", frame)
...
if __name__ == "__main__": print("You are in the wrong directory and are running the wrong file; you must run klaver.py")
